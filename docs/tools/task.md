...
The wire schema is shape-swapped by `task.batch` (default on). One unit of work is the task item `{ name?, agent?, task, complexity, effort?, outputSchema?, schemaMode?, isolated? }`. `isolated` exists only when `task.isolation.enabled` is true **and plan mode is disabled**; `effort` exists only when `task.enableEffort=true` (default off).
...
| `complexity` | `string` | Yes | The caller's terse rationale for why the task is simple or complex (e.g. `one-line rename, no callers`). Rides the child's first prompt into the `auto` thinking classifier as a `<complexity>` state field beside the request; ignored when the child's thinking selector is not `auto` or `effort` overrides it. Blank or missing values are dropped: the schema advertises it as required, but the tool's lenient argument validation still spawns a call that omits it. Item field in batch shape, top-level in flat shape. |
